...
async def registation(info,db):
    ...
    try:
        user_data=model.registered_users(Full_Name=info.Full_Name,Username=info.Username,Email=info.Email,hashed_Password=hashpassword,
                                         Highest_Class=info.Highest_Class,Career_goal=info.Career_goal,University=info.University,CGPA=info.CGPA)
        db.add(user_data)
        db.commit()
...
async def user_login(credential,db):
  ...
  try:  
    ...
    if user:
        if password_hash.verify(credential.password,user.hashed_Password):
            ...
            raise HTTPException(status_code=429,detail="password is wronge")
    else:
